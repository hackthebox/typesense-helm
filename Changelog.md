...
- `pdb.maxUnavailable` default changed from `1` to auto-calculated (`null` in values.yaml)
- `affinity` default changed from `{}` to `null` to enable built-in anti-affinity detection
